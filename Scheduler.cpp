...
void Scheduler::shortestJobFirst() {
    ...
    while (time < totalBurstTime) {
        ...
        if (shortest == n)
            ...
            std::cout << "P" << this->processVector->at(count)->getPID() << ": " << wait << std::endl;
...
void Scheduler::firstComeFirstServe() {
    ...
    for (int i = 0; i < n; i++)
        std::cout << "P" << this->processVector->at(i)->getPID() << ": " << waitTimes[i] << std::endl;
    std::cout << "Average waiting time: " << (totalWait * 1.0) / n << std::endl;
}
...
void Scheduler::roundRobin(int quantum) {
    ...
    for (auto x : schedulingOrder)
        std::cout << this->processVector->at(x)->getPID() << "\t";
