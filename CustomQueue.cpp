...
void CustomQueue::roundRobin(int q) {
    ...
    std::vector<int> rq;//request times
    int     n = (int)this->processVector.size();//number of processes
    //time quantum
    std::vector<int> w;//wait times
    std::vector<int> t;//turn-around times
    std::vector<int> a;//arrival times
    std::vector<int> order;
    
    for (auto x : processVector) {
        a.push_back( x->getArrivalTime() );
        rq.push_back( x->getBurstTime() );
        w.push_back(0);
        t.push_back(0);
    }
    ...
    for (auto x : order )
        std::cout<<x<<"\t";
    std::cout<<"\nAverage turn-around time = "<<tav<<std::endl<<"Average wait time = "<<wav<<std::endl;
}
